...
# 配置日志记录器
if os.path.exists('../logs/compression_ratio_config2.log'):
    os.remove('../logs/compression_ratio_config2.log')
logging.basicConfig(filename='../logs/compression_ratio_config2.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
total_size = min([len(dataset)/compression_ratios[i] for i, dataset in enumerate(datasets)])
logging.info(f"Mixing ratio is {compression_ratios}, Total size is {total_size}")

output_dataset = []
for i, dataset in enumerate(datasets):
    output_dataset.extend(dataset[ : int(total_size * compression_ratios[i])])

with open('../data/compression_ratio_config2/train_data.json', 'w') as file:
...
train_config="../datamix/config/compression_ratio_config2/train.yaml"
...
merge_config="../datamix/config/compression_ratio_config2/merge.yaml"
...
single_file = "../sft_results/compression_ratio_config2/merge/model.safetensors"
...
state_dict_split = split_torch_state_dict_into_shards(
    ...
    filename_pattern="../sft_results/compression_ratio_config2/merge/model-{suffix}.safetensors"
)
...
index_file = "../sft_results/compression_ratio_config2/merge/model.safetensors.index.json"
