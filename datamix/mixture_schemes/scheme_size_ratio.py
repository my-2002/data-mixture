# In this scheme, we sample the data from the data source according to its size ratio
import json
import numpy as np
import random
import logging
...
# 配置日志记录器
if os.path.exists('../logs/size_ratio.log'):
    os.remove('../logs/size_ratio.log')
logging.basicConfig(filename='../logs/size_ratio.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
size_ratios = np.array([9.91, 0.70, 0.21])
size_ratios /= np.sum(size_ratios)
logging.info(f"Mixing ratio is {size_ratios}")

output_dataset = []
for i, dataset in enumerate(datasets):
    output_dataset.extend(dataset[ : int(train_size * size_ratios[i])])

with open('../data/size_ratio/train_data.json', 'w') as file:
...
single_file = "../sft_results/size_ratio/merge/model.safetensors"
...
state_dict_split = split_torch_state_dict_into_shards(
    ...
    filename_pattern="../sft_results/size_ratio/merge/model-{suffix}.safetensors"
)
...
index_file = "../sft_results/size_ratio/merge/model.safetensors.index.json"
