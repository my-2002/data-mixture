# In this scheme, we sample the data from opc-sft-stage2
import json
import numpy as np
import random
import logging
...
# 配置日志记录器
if os.path.exists('../logs/baseline_opc_config2.log'):
    os.remove('../logs/baseline_opc_config2.log')
logging.basicConfig(filename='../logs/baseline_opc_config2.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# 1.混合阶段
logging.info("Mixing phase started")

random.seed(114514)
with open('../source_data/opc-sft-stage2/opc-sft-stage2.json', 'r') as f:
    dataset = json.load(f)
random.shuffle(dataset)
output_dataset = dataset

with open('../data/baseline_opc_config2/train_data.json', 'w') as file:
...
train_config="../datamix/config/baseline_opc_config2/train.yaml"
...
merge_config="../datamix/config/baseline_opc_config2/merge.yaml"
...
single_file = "../sft_results/baseline_opc_config2/merge/model.safetensors"
...
state_dict_split = split_torch_state_dict_into_shards(
    ...
    filename_pattern="../sft_results/baseline_opc_config2/merge/model-{suffix}.safetensors"
)
...
index_file = "../sft_results/baseline_opc_config2/merge/model.safetensors.index.json"
